...
export class PortfolioComponent implements OnInit {
  projects: Project[] = [
    new Project(
      ...
      // tslint:disable-next-line:max-line-length
      'My team and I built a full featured job matching system to intelligently match Jobseekers and Employers based on their resumes and Employer\'s criteria. Individual landing pages were built for each region and industry in Louisiana. Employers were given the ability to create custom landing pages with their own URL. An admin portal was then created to manage logins, view analytics, generate reports and impersonate accounts. In the end we delivered a great product that genuinely wowed our client.',
      // tslint:disable-next-line:max-line-length
    ...
    new Project(
      ...
      // tslint:disable-next-line:max-line-length
      'My team and I designed and developed a desktop web app for 8+ different roles along with an Android app for their field agents to use. In addition to the unique capabilities granted for each role, super admins could impersonate any person, company and role in the system. 14+ hour days for 3+ months became the norm, working with the client every day to confirm and tweak requirements. Each phase took around 7 to 14 days which involved releasing a fully completed feature set for a role. This allowed FEMA\'s agents to begin using the application as soon as possible while continuing work on other parts of the app. The incredibly agile nature of this project proved to be a great challenge, the final product exceeded our client\'s expectations.',
      // tslint:disable-next-line:max-line-length
    ...
    new Project(
      ...
      // tslint:disable-next-line:max-line-length
      'My team and I worked with the client on their expectations and what could be accomplished in the timeframe they have in mind. The client was given the ability to register and customize everything from displaying their own logo in the header/footer to choosing the color scheme of their web app when logged in. A "monitor" could then log in and see the customized Google Maps view showing the live location of every registered device. Admin and super admin roles allowed them to control and edit everything as well as impersonate other accounts/brands. After the main tracking components and apps were completed, an e-commerce portal powered by nopCommerce was developed for the client to automate the selling process.',
      // tslint:disable-next-line:max-line-length
    ...
    new Project(
      ...
      // tslint:disable-next-line:max-line-length
      'A web app accompanied by an iOS and Android app was designed and developed for Prevail Heart Clinics after consultation. The patient is able to easily update their status using the web or mobile app so their doctor can view and assess their well-being remotely. A full featured communications system was also implemented so the patient and doctor could communicate with each other directly from within the app.',
      // tslint:disable-next-line:max-line-length
      'I was responsible for all the front-end aspects of this project. I developed the public facing site using the mockups provided by our designers. The biggest challenge came with the internal web app development. The back-end development was moving faster than I was so many of the views were being generated first and I went in afterwards to integrate them. It was a chance to think of front-end integration in a different way. A few months after the site and application launched, the client went through a rebranding and I was tasked with handling all the UI rebranding for the web app.',
    ...
    new Project(
      ...
      // tslint:disable-next-line:max-line-length
      'After consulting and brainstorming with the client, my team and I went to work on the redesign. The decision was made to use a pre-packaged CMS, DotNetNuke (DNN) and integrate customized templates and modules into it. DNN integrated well with the framework being using and also provided a great out of the box experience for content/account management. A calendar module was developed with the ability to assign custom tags for events. An event request system with the ability to save and finish later was created to simplify and expedite the event approval process.',
      // tslint:disable-next-line:max-line-length
